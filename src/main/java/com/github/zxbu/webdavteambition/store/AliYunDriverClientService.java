...
/*        FileListRequest listQuery = new FileListRequest();
        ...
        params.put("mediaType","0");
        params.put("folderId",nodeId);
        params.put("iconOption","5");
        params.put("orderBy","lastOpTime");
        params.put("descending","true");

        String json = client.get("/open/file/listFiles.action", params);
        String fileJson = JsonUtil.toJson(JsonUtil.getJsonNodeValue(json,"fileListAO"));
        TFileListResult<TFile> tFileListResult = JsonUtil.readValue(fileJson, new TypeReference<TFileListResult<TFile>>() {
        });
        //all.addAll(tFileListResult.getFileList());
        for (TFile tFile : tFileListResult.getFileList()) {
            //临时补丁
            if (tFile.getType() == null) {
                if(tFile.getMd5() == null){
                    tFile.setType(FileType.folder.name());
        ...
        //all.addAll(tFileListResult.getFolderList());
        for (TFile tFile : tFileListResult.getFolderList()) {
            //临时补丁
            if (tFile.getType() == null) {
                if(tFile.getMd5() == null){
                    tFile.setType(FileType.folder.name());
        ...
        if (!(tFileListResult.getFileListSize()>60)) {
            return all;
        }
        return fileListFromApi(nodeId, "2", all);
    }
...
//        uploadPreRequest.setContent_hash(UUID.randomUUID().toString());
    ...
    public void createFolder(String path) {
        ...
        if (createFileResult.getName() == null) {
    ...
    public Response download(String path, HttpServletRequest request, long size ) {
        ...
        //String json = client.post("/", downloadRequest);
        Map<String,String> params = new HashMap<>();
        params.put("fileId",file.getId());
        String json = client.get("/open/file/getFileDownloadUrl.action",params);
        Object url = JsonUtil.getJsonNodeValue(json, "fileDownloadUrl");
